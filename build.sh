...
go build -o dist/network-scanner

# Linux 64-bit
echo "Сборка для Linux 64-bit..."
GOOS=linux GOARCH=amd64 go build -o dist/network-scanner-linux-amd64

# Windows 64-bit
echo "Сборка для Windows 64-bit..."
GOOS=windows GOARCH=amd64 go build -o dist/network-scanner-windows-amd64.exe

# macOS Intel
echo "Сборка для macOS Intel..."
GOOS=darwin GOARCH=amd64 go build -o dist/network-scanner-darwin-amd64

# macOS Apple Silicon
echo "Сборка для macOS Apple Silicon..."
GOOS=darwin GOARCH=arm64 go build -o dist/network-scanner-darwin-arm64
