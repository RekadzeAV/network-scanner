...
- **Топология:** боковая панель разбита на карточки «Подключение (SNMP)», «Отчёты и превью», «Интерактивная карта»
- **Результаты:** редкие настройки (режим, сортировка, CIDR, пресеты, быстрые фильтры, Inventory DB) вынесены в Accordion «Фильтры и режимы отображения»; в подрежиме `Inventory` сворачивается автоматически
- **Валидация ввода on-the-fly:** CIDR и диапазон портов проверяются при вводе (`configvalidation`), ошибка показывается под полем цветом ошибки темы
- **Статус-бар:** единая точка обратной связи `setStatus`; неблокирующие подтверждения `setStatusToast` (маркер «✓», авто-возврат 4 с)
- **Меню «Справка»:** «Горячие клавиши» (F1) и «О программе» (версия/ОС/Go)
