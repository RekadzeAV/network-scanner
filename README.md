...
| Задач v2.1 backlog | 27/28 выполнено (96%) | ✅ |
...
|----------|---------|---------|
| `Error: permission denied` при сканировании | Недостаточно прав для создания raw-сокетов | Запустите от `root`/администратора |
| MAC-адреса не определяются | ARP-таблица пуста или нет доступа | Используйте `--topology` с SNMP или запустите с повышенными правами |
| Graphviz не найден при экспорте PNG/SVG | Graphviz не установлен в PATH | Установите [Graphviz](https://graphviz.org/download/) и добавьте в PATH |
| GUI не запускается (нет окна) | Отсутствует C-компилятор для CGO | Установите [MinGW-w64](https://www.mingw-w64.org/) или [TDM-GCC](https://jmeubank.github.io/tdm-gcc/) |
| Медленное сканирование | Большой диапазон портов + много хостов | Уменьшите `--port-range` или используйте `--auto-profile` |
| Ошибка `timeout` при SNMP | Неверная community или SNMP не включён | Проверьте community-строку и включите SNMP на устройствах |
| Ошибка `context deadline exceeded` | Таймаут слишком маленький для большой сети | Увеличьте `--timeout` (по умолчанию 2 сек) |

### Диагностика

Для получения диагностической информации запустите сканирование с флагами:

```bash
# Подробные логи по каждому порту
./network-scanner --verbose-port-logs

# Расширенные эвристики определения ОС
./network-scanner --os-detect-active
