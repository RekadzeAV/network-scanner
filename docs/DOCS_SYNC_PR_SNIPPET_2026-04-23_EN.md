...
- Standardized CLI examples across docs to `--network`, `--ports`, `--timeout`, `--threads`, `--show-closed`, `--help`, `--version`.
- Updated install/cross-compilation documentation baseline to `Go 1.24+`.
- Updated Docker examples from `golang:1.21` to `golang:1.24`.
