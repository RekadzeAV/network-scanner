...
**Статус выполнения:** 🟡 В процессе (D4 + D1 smoke-тесты завершены)

**Цель:** повысить точность и воспроизводимость построения топологии.

- Качество данных:
  - [x] улучшение сопоставления LLDP/FDB и дедупликация связей;
  - [x] уверенность связей (`high/medium/low`) с прозрачными правилами;
  - [x] диагностика "почему связь построена" (ExplainLink, DedupReport);
  - [ ] тест-кейсы с частичным SNMP-ответом и mixed-vendor окружением.
- Надежность рендера:
  - [x] стабилизация Graphviz-path (`dot`) и fallback при отсутствии бинаря;
  - [x] graceful degradation в GUI (текстовая топология при недоступном рендере).
- Тестирование:
  - [x] smoke-тесты для дедупликации, explain, текстового экспорта;
