...
func TestIntegrationOperationsManager_FullLifecycle(t *testing.T) {
	...
	// failedTask пишется из горутины execute(), читать его надо под мьютексом
	failedTaskMu.Lock()
	ranFailedTask := failedTask
	failedTaskMu.Unlock()
	if !ranFailedTask {
...
func TestIntegrationOperationsManager_Cancellation(t *testing.T) {
	...
	// Сигналим из горутины задачи вместо флага: флаг читался бы без
	// синхронизации, а ожидание по каналу убирает и гонку, и плавающий Sleep.
	...
	id := mgr.Run(OperationTypeTool, "Cancellable Operation", func(ctx context.Context) error {
		select {
		case <-ctx.Done():
			signalOnce.Do(func() { close(cancelled) })
			return ctx.Err()
