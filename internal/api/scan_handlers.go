...
func (h *Handler) handleScan(w http.ResponseWriter, r *http.Request) {
	...
	//
	// cancel вызывается по завершении фонового сканирования (см. горутину ниже),
	// а не при возврате обработчика: handleScan отвечает 202 немедленно, и
	// cancel здесь отменил бы сканирование раньше, чем оно реально начнётся.
	...
	go func() {
		// Освобождаем ресурс контекста по завершении сканирования.
		// cancel идемпотентен, поэтому CancelScan может вызывать его тоже.
		defer cancel()
