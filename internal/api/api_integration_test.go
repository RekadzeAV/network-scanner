...
// withAlertingEngine явно инициализирует глобальный движок алертинга на время
// теста и восстанавливает прежнее значение.
//
// alertingEng — пакетная глобальная переменная: другие тесты оставляют её
// инициализированной (initAlerting) либо nil, поэтому зависеть от её состояния
// нельзя — иначе обработчик уходит либо в 503, либо в загрузку снапшотов.
func withAlertingEngine(t *testing.T) {
	t.Helper()

	alertingEngMu.Lock()
	original := alertingEng
	alertingEngMu.Unlock()

	initAlerting(filepath.Join(t.TempDir(), "alerts.log"))

	t.Cleanup(func() {
		alertingEngMu.Lock()
		alertingEng = original
		alertingEngMu.Unlock()
	})
}

// seedInventory записывает снапшоты во временную БД инвентаря.
func seedInventory(t *testing.T, path string, snapshots ...inventory.Snapshot) {
	t.Helper()
	...
	for _, snapshot := range snapshots {
		if err := store.SaveSnapshot(snapshot.ID, snapshot.Timestamp, snapshot.Hosts); err != nil {
			t.Fatalf("save snapshot %s: %v", snapshot.ID, err)
		}
	}
}
...
func TestIntegrationAlertsTrigger_Valid(t *testing.T) {
	...
	// Инвентарь — во временном каталоге, чтобы тест не трогал рабочие данные.
	...
	seedInventory(t, dbPath,
		inventory.Snapshot{
			...
			Hosts:     []scanner.Result{{IP: "192.168.1.10", Hostname: "host-a", IsAlive: true}},
		},
		inventory.Snapshot{
			ID:        "scan-002",
			Timestamp: now,
			Hosts: []scanner.Result{
				{IP: "192.168.1.10", Hostname: "host-a", IsAlive: true},
				{IP: "192.168.1.20", Hostname: "host-b", IsAlive: true}, // новый хост → сработает rule-001
			},
		},
	)
	...
	// Проверяем точный успешный код: 404 возвращает не только роутер, но и сам
	// обработчик (снапшот не найден), так что «не 404» про бы эту регрессию.
	if w.Code != http.StatusOK {
	...
	if resp.Count < 1 || len(resp.Alerts) != resp.Count {
		t.Errorf("expected alerts for the new host, got count=%d len=%d", resp.Count, len(resp.Alerts))
	}
	if resp.ScanA != "scan-001" || resp.ScanB != "scan-002" {
		t.Errorf("echoed scan ids = %q / %q", resp.ScanA, resp.ScanB)
	}
}
